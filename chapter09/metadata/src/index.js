...
function connectRabbit(rabbitHost) {
  ...
  return amqp
    .connect(rabbitHost) // Connect to the RabbitMQ server.
...
function setupHandlers(microservice) {
  ...
  microservice.app.get("/videos", (req, res) => {
    return videosCollection
      .find() // Returns a promise so we can await the result in the test.
  ...
  microservice.app.get("/video", (req, res) => {
    ...
    return videosCollection
      .findOne({ _id: videoId }) // Returns a promise so we can await the result in the test.
      .then((video) => {
        if (!video) {
  ...
  function consumeVideoUploadedMessage(msg) {
    ...
    return videosCollection
      .insertOne(videoMetadata) // Record the metadata for the video.
      .then(() => {
  ...
  return microservice.messageChannel
    .assertExchange("video-uploaded", "fanout") // Assert that we have a "video-uploaded" exchange.
    .then(() => {
    ...
    .then((response) => {
      ...
      return microservice.messageChannel
        .bindQueue(queueName, "video-uploaded", "") // Bind the queue to the exchange.
        .then(() => {
...
function startHttpServer(dbConn, messageChannel) {
  return new Promise((resolve) => {
    ...
    const app = express();
    const microservice = {
      // Create an object to represent our microservice.
      app: app,
      db: dbConn.db,
